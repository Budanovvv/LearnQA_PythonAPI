...
class TestUserGet(BaseCase, Assertions):
    ...
    def test_get_user_get_details_auth_is_same_user(self):
        response2 = requests.get(f"https://playground.learnqa.ru/api/user/{self.user_id_from_auth_method}",
    ...
    def test_auth_user_try_to_get_details_another_user(self):
        response2 = requests.get(f"https://playground.learnqa.ru/api/user/{self.user_id_from_auth_method + 1}",
                                 cookies={"auth_sid": self.auth_sid},
                                 headers={"x-csrf-token": self.token}
                                 )
